...
rr = as_tibble(aa) %>% 
  filter(start_date >= ymd("2020-05-01"), is_active == 1) %>%
...
bb <- fires_21_0822 %>%  
  mutate(
    Amazon = case_when(
      biome == 1 ~ "inside Amazon Biome"
      biome == 0 ~ "outside Amazon Biome"
  ),
  start_date = ymd(start_date),
  last_date = ymd(last_date),
  Year = year(start_date),
